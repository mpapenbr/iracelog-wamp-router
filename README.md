...
[![Build Status](https://img.shields.io/github/checks-status/mpapenbr/iracelog-wamp-router/main?color=black&style=for-the-badge&logo=github)][github-actions]
...
[![License](https://img.shields.io/github/license/mpapenbr/iracelog-wamp-router?color=red&style=for-the-badge)][project-license]
[![Go v1.20](https://img.shields.io/badge/Go-%20v1.20-black?style=for-the-badge&logo=go)][gomod-file]

WAMP Router for the iRacelog project
...
`iracelog-wamp-router` on their end
...
```sh
git clone https://github.com/mpapenbr/iracelog-wamp-router
cd iracelog-wamp-router
```
...
After cloning the repository you already changed into the project dir iracelog-wamp-router.
...
This section will guide you to setup a fully-functional local copy of `iracelog-wamp-router`
...
from [`iracelog-wamp-router's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `iracelog-wamp-router`, run the
command
...
The pre-commit hooks used by `iracelog-wamp-router` are located within the
...
Tests in `iracelog-wamp-router` are classified as _fast_ and _slow_ - depending
...
| `run` | Run _iracelog-wamp-router_ | NA |
...
### Running `iracelog-wamp-router`

To run iracelog-wamp-router, use the command
...
`iracelog-wamp-router` is powered by a template generated using [`cookiecutter-go`][cookiecutter-go-link] which is based on [`go-template`][go-template-link]
